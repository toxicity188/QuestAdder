...
data class MushroomStemBlockData(
    val west: Boolean,
    val up: Boolean,
    val south: Boolean,
    val north: Boolean,
    ...
    companion object {
        fun fromHash(hash: Int): MushroomStemBlockData {
            val bit = hash and ((1 shl 6) - 1)

            return MushroomStemBlockData(
                (bit and 1) == 1,
                ((bit ushr 1) and 1) == 1,
                ((bit ushr 2) and 1) == 1,
                ((bit ushr 3) and 1) == 1,
        ...
        fun fromBlock(blockData: MultipleFacing): MushroomStemBlockData {
    ...
    override fun createBlockData(): BlockData = (Material.MUSHROOM_STEM.createBlockData() as MultipleFacing).also {
