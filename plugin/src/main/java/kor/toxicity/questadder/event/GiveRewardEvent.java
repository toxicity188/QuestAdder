...
public class GiveRewardEvent extends QuestAdderPlayerEvent implements Cancellable {
    ...
    public GiveRewardEvent(@NotNull Player who, RewardSet rewardSet) {
        super(who);
        exp = rewardSet.getExp();
        money = rewardSet.getMoney();
        itemStacks = Arrays.stream(rewardSet.getItems()).filter(r -> ThreadLocalRandom.current().nextDouble(0,100) <= r.getChance()).map(RewardSet.RewardSetItem::getItem).collect(Collectors.toList());
    }
