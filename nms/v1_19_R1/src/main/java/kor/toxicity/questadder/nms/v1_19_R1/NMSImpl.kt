package kor.toxicity.questadder.nms.v1_19_R1
...
class NMSImpl: NMS {
    ...
    private class VirtualPlayerImpl(player: Player, location: Location, skin: GameProfile): VirtualEntityImpl<EntityPlayer>(player,
        EntityPlayer((Bukkit.getServer() as CraftServer).server,(location.world as CraftWorld).handle,skin,null).apply {
        a(location.x,location.y,location.z,location.yaw,location.pitch)
    }), VirtualPlayer {
        init {
    ...
    override fun getGameProfile(player: Player): GameProfile {
        return (player as CraftPlayer).handle.fy()
    }
    ...
    private class FakeItemImpl(itemStack: ItemStack, location: Location): FakeItem {
        ...
        val handle = CraftItem(Bukkit.getServer() as CraftServer, EntityItem(EntityTypes.T, world).apply {
            ap = 10
            a(location.x, location.y, location.z, location.yaw, location.pitch)
